...
class TemporalBridge {
    ...
    async waitForStatus(workflowId) {
        await this.client.startWorkflow(wfId, {
            taskQueue: queue,
    ...
    async provision(clusterName, provider) {
        ...
        const handle = await this.client.startWorkflow(wfId, {
            taskQueue: queue,
            args: { workflowId: wfId, name: clusterName, provider, logFile: `${Date.now()}-${Math.random().toString(36).slice(2)}-A1.log` },
        });
    ...
    async destroyCluster(clusterId) {
        ...
        const handle = await this.client.startWorkflow(wfId, {
            taskQueue: queue,
            args: { workflowId: wfId, name: cluster.name, provider: cluster.provider, logFile: `${Date.now()}-destroy-${Math.random().toString(36).slice(2)}-B2.log` },
        });
