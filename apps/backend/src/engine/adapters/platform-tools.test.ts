...
describe('search_web', () => {
  const web = (over: Partial<{ search: unknown; fetchPage: unknown }> = {}) => ({
    ...
    fetchPage: vi.fn(async () => 'the page body'),
    ...over,
  }) as never;
  ...
  it('fetches a page and needs a url', async () => {
    ...
    expect((await run(tools, 'fetch_web_page', { url: 'https://example.test' })).digest).toBe('the page body');
...
describe('what gets registered', () => {
  ...
  it('offers only what it was given', () => {
    const webOnly = createPlatformTools({
      web: { search: vi.fn(async () => ({ results: [] })), fetchPage: vi.fn(async () => '') },
