...
import { agentTools, needsWorkspace, type AgentDefinition, type EgressMode } from './agent.js';

export type { EgressMode } from './agent.js';
...
function describeEnvironment(request: ContextRequest): string {
  ...
  const delegates = (request.callable ?? []).some(needsWorkspace);
...
function describeAsks(environment: ResolvedEnvironment, tools: readonly ToolContract[]): string {
...
function describeWithheld(withheld: readonly WithheldTool[]): string {
...
function describeAvailable(tools: readonly ToolContract[]): string {
  ...
  return [
    ...
    ...tools.map((tool) => {
      ...
      return `- ${tool.name}: ${tool.description}${guidance ? ` ${guidance}` : ''}`;
    }),
  ].join('\n');
}

export function composeContext(request: ContextRequest): ComposedContext {
  ...
  const sections = [
    request.agent.prompt.trim(),
    describeEnvironment(request),
    describeAvailable(tools),
    describeWithheld(withheld),
    describeAsks(request.environment, tools),
    request.memory?.trim() ?? '',
    outputs.length > 0
      ? `When you are done, your answer must provide: ${outputs.join(', ')}.`
      : '',
  ];

  return { text: sections.filter((section) => section.length > 0).join('\n\n'), tools, withheld };
}
