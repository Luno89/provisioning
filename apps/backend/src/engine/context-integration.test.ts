...
import { createEngineActivities } from './temporal/activities.js';
import { createAgentRegistry } from './adapters/registry.js';
import { createEnvironmentResolver, type WorkspaceSource } from './adapters/environments.js';
import { createRunEnvironments } from './adapters/run-environments.js';
...
import type { ModelCallArgs, RunTicket } from './temporal/contracts.js';
...
async function callWith(
  activities: ReturnType<typeof engine>,
  run: RunTicket,
  over: Partial<ModelCallArgs> = {},
) {
  const fetchImpl = stubModel();
  const environment = await activities.EngineResolveEnvironmentActivity({ ticket: run });

  await activities.EngineModelCallActivity({
    ticket: run,
    nodeId: 'think',
    environment,
    tools: 'granted',
