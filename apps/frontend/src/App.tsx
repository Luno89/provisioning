...
import { Activity, AlertTriangle, ArrowLeft, ArrowRight, BellRing, Box, Check, Cloud, Cpu, Database, ExternalLink, FileText, GitBranch, HardDrive, Key, Layers, Loader2, Network, Package, Puzzle, RefreshCw, Server, Settings, Shield, Terminal, Timer, Trash2, X, Zap } from 'lucide-react';
...
import { TABBY_TOOL_FORMATS } from './components/app-catalog';
import { APP_DEFAULTS } from './components/app-catalog';
...
function App() {
  ...
  /**
   * The TabbyAPI image picker appears here too — on a running TabbyAPI deployment — not only in the
   * deploy wizard. That is why the query is a shared hook rather than wizard state; see
   * `useTabbyImageTags`. This condition is the second half of what used to be one
   * `tabbyImagePickerOpen` OR-ing both screens together, which is exactly why moving the query into
   * the wizard would have broken this one silently.
   */
  const { options: tabbyImageTagOptions, loading: loadingTabbyImageTags } = useTabbyImageTags(
    !!currentDeployment && currentDeployment.appType === 'tabbyapi',
  );
