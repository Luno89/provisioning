...
export const hasDatabase = (appType: string, catalogue: readonly CatalogueEntry[] = []): boolean =>
  Boolean(catalogue.find((c) => c.id === appType)?.uiDefaults?.hasDatabase);
...
export function nextStep(
  step: number, appType: string, isCatalogueApp = false, catalogue: readonly CatalogueEntry[] = [],
): number {
  if (isCatalogueApp) return step === 1 ? LAST_STEP : Math.min(step + 1, LAST_STEP);
  if (step === 2) return isModelApp(appType) ? 3 : 4;
  if (step === 3) return 4;
  if (step === 4 && !hasDatabase(appType, catalogue)) return 6;
...
export function prevStep(
  ...
  if (step === 3) return 2;
  if (step === 6 && !hasDatabase(appType, catalogue)) return 4;
