...
import { Plus, Loader2, Trash2, ChevronRight, ChevronDown, CircleSlash, Link2, Unlink, Check, Sprout } from 'lucide-react';
...
interface Leaf {
  ...
  body?: string;
  column: ColumnId;
  status: 'proposed' | 'pending' | 'running' | 'succeeded' | 'failed' | 'cancelled';
  parentLeafId?: string;
  depth: number;
  blocking: boolean;
  childCount: number;
 ...
 *
 * Theming the wire format would make logs, errors and any future integration read as riddles.
 * The koala vocabulary belongs in the UI, where it is atmosphere, not in the contract.
 */
const STATUS_LABEL: Record<Leaf['status'], string> = {
  proposed: 'Sprouting',
  pending: 'Ripe',
  running: 'Munching',
  succeeded: 'Digested',
...
export default function Board({ apiBase }: { apiBase: string }) {
  ...
  const renderLeaf = (leaf: Leaf, nested = false) => {
    ...
    return (
      <div key={leaf.id} className={`rounded-xl border ${nested ? 'border-slate-800 bg-slate-950' : 'border-slate-800 bg-slate-900'} p-3`}>
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              <span
                className={`text-[10px] uppercase tracking-widest ${STATUS_STYLE[leaf.status]}`}
                title={leaf.status}
              >
          ...
          <div className="flex items-center gap-1 shrink-0">
            {leaf.status === 'proposed' && (
              <button onClick={() => acceptLeaf.mutate(leaf.id)} title="Accept — starts the work" className="text-emerald-500 hover:text-emerald-300"><Check size={14} /></button>
            )}
            {leaf.status === 'running' && (
              <button onClick={() => cancelLeaf.mutate(leaf.id)} title="Cancel" className="text-slate-600 hover:text-amber-400"><CircleSlash size={13} /></button>
            )}
        ...
        {!derived && (
          <select
            value={leaf.column}
            onChange={(e) => moveLeaf.mutate({ id: leaf.id, column: e.target.value as ColumnId })}
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && childTitle.trim()) {
                    createLeaf.mutate({ title: childTitle, parentLeafId: leaf.id, blocking: childBlocking, column: 'todo' });
                  }
        ...
        {isOpen && kids.length > 0 && (
          <div className="mt-2 space-y-2 pl-3 border-l border-slate-800">{kids.map((k) => renderLeaf(k, true))}</div>
        )}
      </div>
    );
  };
    ...
    <div>
      ...
      <div className="flex gap-3 mb-6 max-w-2xl">
        <input
          ...
          onKeyDown={(e) => { if (e.key === 'Enter' && newTitle.trim()) createLeaf.mutate({ title: newTitle, column: 'todo' }); }}
        ...
        />
        <button
          onClick={() => createLeaf.mutate({ title: newTitle, column: 'todo' })}
          disabled={!newTitle.trim() || createLeaf.isPending}
          className="px-4 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-40 flex items-center gap-2 text-sm"
        >
          {createLeaf.isPending ? <Loader2 className="animate-spin" size={14} /> : <Plus size={14} />} Add
        </button>
      </div>
      ...
      {roots.some((c) => c.status === 'proposed') && (
        <div className="mb-6 rounded-xl border border-emerald-900/60 bg-emerald-950/20 p-4">
          <h3 className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-2 flex items-center gap-2">
            <Sprout size={12} /> Sprouting — proposed, not started
          </h3>
          {/* Proposals sit outside the columns on purpose: a column is where work SITS, and a
              proposal is work that does not exist until someone accepts it. */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {roots.filter((c) => c.status === 'proposed').map((c) => renderLeaf(c))}
          </div>
        </div>
      )}
