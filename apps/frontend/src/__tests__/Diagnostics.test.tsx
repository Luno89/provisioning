...
describe('Helm Status & Diagnostics', () => {
  it('shows diagnostics when the tab is clicked', async () => {
    const queryClient = createTestQueryClient();
    vi.clearAllMocks();
    
    mockedAxios.get.mockImplementation((url) => {
      if (url.includes('/diagnostics')) {
        return Promise.resolve({ data: { content: '--- POD STATUS ---\npod-odoo-running' } });
      }
      if (url.includes('/helm')) {
        return Promise.resolve({ data: { content: 'STATUS: deployed' } });
      }
      if (url.includes('/deployments')) {
        return Promise.resolve({ data: [{ id: 'd1', name: 'Odoo', clusterId: 'c1', status: 'running', strategy: 'native' }] });
      }
    ...
    const diagTab = await screen.findByText(/Diagnostics/i);
    await user.click(diagTab);

    // Check for diagnostic content
    await waitFor(() => {
      expect(screen.getByText(/pod-odoo-running/i)).toBeInTheDocument();
    }, { timeout: 10000 });
  });
});
