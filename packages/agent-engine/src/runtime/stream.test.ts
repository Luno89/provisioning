...
describe('createStreamParser', () => {
  ...
  it('captures error payloads sent by OpenAI-compatible endpoints', () => {
    const events = drain([
      frame({ error: { message: 'Chat completion aborted. Please check the server console.' } }),
    ]);
    expect(events).toEqual([
      { kind: 'error', error: 'Chat completion aborted. Please check the server console.' },
    ]);
  });
