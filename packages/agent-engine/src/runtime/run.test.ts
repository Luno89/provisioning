...
describe('budgetExceeded', () => {
  ...
  it('only holds a limit against a step that would spend it, while time always counts', () => {
    const state = createRunState(0);
    recordToolOutcome(state, { callId: 'c', name: 't', ok: true, digest: '' }, 0);

    expect(budgetExceeded(state, { maxToolCalls: 1 }, identity(), 0, [])).toBeUndefined();
    expect(budgetExceeded(state, { maxToolCalls: 1 }, identity(), 0, ['rounds'])).toBeUndefined();
    expect(budgetExceeded(state, { maxToolCalls: 1 }, identity(), 0, ['toolCalls'])).toMatch(/tool calls/);
