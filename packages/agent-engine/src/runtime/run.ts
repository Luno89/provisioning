...
import type { ModelCallResult } from '../model/model-call.js';
...
export function applyModelResult(state: RunState, result: ModelCallResult, now: number = Date.now()): RunState {
  ...
  state.counters.longestReply = Math.max(state.counters.longestReply, numberOf(result.usage?.completion_tokens));
...
export const BUDGET_SPENDS = ['rounds', 'tokens', 'toolCalls', 'childRuns'] as const;

export type BudgetSpend = (typeof BUDGET_SPENDS)[number];
...
export function budgetExceeded(
  ...
  now: number = Date.now(),
  spends: readonly BudgetSpend[] = BUDGET_SPENDS,
): string | undefined {
  const { counters } = state;
  const checks = new Set(spends);
  ...
  if (checks.has('rounds') && budget.maxRounds !== undefined && counters.rounds >= budget.maxRounds) {
    return `used all ${budget.maxRounds} rounds`;
  }
  if (checks.has('tokens') && budget.maxTokens !== undefined && counters.totalTokens >= budget.maxTokens) {
    return `spent ${counters.totalTokens} tokens, past the limit of ${budget.maxTokens}`;
  }
  if (checks.has('toolCalls') && budget.maxToolCalls !== undefined && counters.toolCalls >= budget.maxToolCalls) {
    return `made ${counters.toolCalls} tool calls, the limit is ${budget.maxToolCalls}`;
  }
  if (checks.has('childRuns') && budget.maxChildRuns !== undefined && counters.childRuns >= budget.maxChildRuns) {
