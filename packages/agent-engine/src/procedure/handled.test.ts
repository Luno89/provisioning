...
describe('the steps a procedure does for the model', () => {
  it('takes every tool call as the procedure’s own unless it is shared', () => {
    const found = handledSteps(procedure([
      node({ id: 'claim', kind: 'call-tool', settings: { tool: 'start_task', says: 'The task has already been claimed for you.' } }),
      node({ id: 'check', kind: 'call-tool', settings: { tool: 'run_command', shared: true } }),
  ...
  it('leaves a shared tool for the model to choose as well', () => {
  ...
  it('ignores a step that names no tool at all', () => {
    expect(handledSteps(procedure([node({ id: 'broken', kind: 'call-tool', settings: {} })]))).toEqual([]);
  });

  it('falls back to naming the tool when the step says nothing', () => {
    const found = handledTools(procedure([
      node({ id: 'record', kind: 'call-tool', settings: { tool: 'mark_done' } }),
    ]));
  ...
  it('joins two steps that handle the same tool differently', () => {
    const found = handledTools(procedure([
      node({ id: 'rejected', kind: 'call-tool', settings: { tool: 'mark_failed', says: 'A rejected verdict is recorded for you.' } }),
      node({ id: 'unjudged', kind: 'call-tool', settings: { tool: 'mark_failed', says: 'Work that could not be judged is recorded for you.' } }),
    ]));
  ...
  it('says the same thing once when two steps say it identically', () => {
    const found = handledTools(procedure([
      node({ id: 'a', kind: 'call-tool', settings: { tool: 'mark_failed', says: 'The outcome is recorded for you.' } }),
      node({ id: 'b', kind: 'call-tool', settings: { tool: 'mark_failed', says: 'The outcome is recorded for you.' } }),
    ]));
...
describe('telling the model what happens around it', () => {
  it('writes a section naming each thing done for it', () => {
    const text = describeHandledSteps(procedure([
      node({ id: 'claim', kind: 'call-tool', settings: { tool: 'start_task', says: 'The task has already been claimed for you.' } }),
      node({ id: 'record', kind: 'call-tool', settings: { tool: 'mark_done', says: 'The outcome is recorded for you.' } }),
    ]));
...
describe('a persona the procedure hands work to itself', () => {
  it('is found the same way a tool is, by the agent it names', () => {
    const found = handledTools(procedure([
      node({ id: 'judge', kind: 'delegate', settings: { agent: 'judge', says: 'A judge weighs your work when you finish.' } }),
    ]));

    expect(found.judge).toBe('A judge weighs your work when you finish.');
  });

  it('is left offered when the step is shared with the model', () => {
    const found = handledTools(procedure([
      node({ id: 'judge', kind: 'delegate', settings: { agent: 'judge', shared: true } }),
    ]));
  ...
  it('ignores a kind of step that hands nothing over', () => {
    expect(handledSteps(procedure([node({ id: 'x', kind: 'fan-out', settings: { agent: 'judge' } })]))).toEqual([]);
  });
});
