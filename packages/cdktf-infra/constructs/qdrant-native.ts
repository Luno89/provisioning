...
/**
 * Qdrant — vector search, for the part of the corpus worth embedding.
 *
 * ── WHY IT IS NOT THE WHOLE CORPUS ──
 * A terabyte of crawled text is roughly 1.5 billion 800-character chunks. As float32 vectors at 768
 * dimensions that is 4.7 TB — an index several times larger than the thing it indexes. Even binary
 * quantized and kept in RAM it is ~148 GB.
 *
 * So embedding everything costs more than storing everything, and this is deliberately the second
 * tier: Quickwit indexes all of it for exact terms, and Qdrant holds the working set a project
 * actually pulled in. What decides membership is the ingest's scope, not the crawler.
 *
 * ── THE QUANTIZATION SETTINGS ARE PER-COLLECTION, NOT HERE ──
 * Binary quantization with originals `on_disk` is what makes the RAM figure above survivable, and
 * it is set when a collection is created rather than by any flag on this pod. This construct's job
 * is to give that collection somewhere to live with enough disk for the originals it pushes off
 * memory — hence a PVC sized well above the vectors themselves.
 *
 * ── 6333 IS HTTP, 6334 IS gRPC ──
 * Both are exposed. The client here speaks HTTP, but a bulk upsert of millions of points is the one
 * case where gRPC is worth having available without redeploying.
 */
export interface QdrantNativeConfig extends VpnConfig {
  readonly namespace?: string;
  readonly webRepo?: string;
  readonly webTag?: string;
  readonly serviceType?: string;
  /** Generated per-deployment when absent. Sent as the `api-key` header. */
  readonly apiKey?: string;
  readonly storage?: string;
  /**
   * Memory ceiling. Qdrant holds quantized vectors and the HNSW graph in RAM; the originals go to
   * disk. Too low does not degrade gracefully — the process is OOMKilled mid-search.
   */
  readonly memoryLimit?: string;
}

export class QdrantNativeApp extends Construct {
  constructor(scope: Construct, id: string, config: QdrantNativeConfig = {}) {
    super(scope, id);
