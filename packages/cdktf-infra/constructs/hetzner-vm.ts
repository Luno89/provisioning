...
export interface HetznerVmConfig {
  /** Cluster name — also the VM's name and the prefix for every sibling resource. */
  readonly name: string;
  /** SSH public key injected at boot; its private half does the k3s bootstrap over SSH. */
...
export class HetznerVmStack extends TerraformStack {
  constructor(scope: Construct, id: string, config: HetznerVmConfig) {
    super(scope, id);
