...
/**
 * SearXNG — a metasearch front end, deployed so the agent has a real search API.
 *
 * ── WHY THIS IS DEPLOYABLE RATHER THAN A LIBRARY CALL ──
 * The built-in search parses DuckDuckGo's HTML with a regex. It returns five results wrapped in
 * `duckduckgo.com/l/?uddg=` redirects, and it breaks silently whenever DuckDuckGo touches its
 * markup. Measured against this construct's config on the query the planner actually failed on
 * ("github api create pull request"): 29 results, real destination URLs, real snippets, from
 * several engines at once. No API key anywhere — that is the reason to run one rather than pay
 * for a search API.
 *
 * ── THE ONE SETTING THAT MATTERS ──
 * SearXNG ships `formats: [html]`. The JSON API is OFF by default and answers 403 until it is
 * turned on, so a deployment left at defaults looks healthy, serves its web UI, and is useless to
 * a program. Enabling it needs a settings file, which is the only reason this construct mounts
 * one at all.
 */
export interface SearxngNativeConfig extends VpnConfig {
  readonly namespace?: string;
  readonly webRepo?: string;
  readonly webTag?: string;
  readonly serviceType?: string;
  /**
   * Signs session cookies. Generated per-deployment when absent.
   *
   * The image's own entrypoint only substitutes its `ultrasecretkey` placeholder when it CREATES
   * the settings file from its template — mounting one, as this does, skips that path entirely and
   * would otherwise ship the published placeholder as the live key.
   */
  readonly secretKey?: string;
  /** Comma-separated engine names to restrict to. Empty means SearXNG's own default set. */
  readonly engines?: string;
}

export class SearxngNativeApp extends Construct {
  constructor(scope: Construct, id: string, config: SearxngNativeConfig = {}) {
    super(scope, id);
