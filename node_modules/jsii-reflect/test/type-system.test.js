...
test('Submodules can have a README', () => {
    ...
    const isolated = submodule?.submodules.find((m) => m.fqn === 'jsii-calc.submodule.isolated');
    expect(submodule?.readme?.markdown).toMatch(/This is the readme.*jsii-calc.submodule/);
    expect(isolated?.readme?.markdown).toMatch(/This is the readme.*jsii-calc.submodule.isolated/);
});
test('overridden member knows about both parent types', () => {
    const ts = (0, util_1.typeSystemFromSource)(`
    export class Foo {
      public bar() {
        Array.isArray(3);
      }
...
test('Assembly allTypes includes submodule types', () => {
    const asm = (0, util_1.assemblyFromSource)({
